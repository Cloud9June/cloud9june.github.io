...
const CONFIG = {
    ...
    // ===== 교직원 내선번호 → 외부 발신 번호 =====
    // 학교 밖(휴대폰)에서 걸 때 필요한 전체 번호 규칙입니다.
    // {ext} 자리에 내선번호가 들어갑니다.  예) 내선 308 → 031-720-1308
    //
    // ⚠️ 이 규칙을 따르지 않는 번호가 있으면 staff-data.js 의 해당 항목에
    //    "tel": "031-720-0000" 처럼 직접 적어 두면 그 값이 우선합니다.
    phone: {
        dialPattern: '0317201{ext}',      // 실제로 거는 번호 (숫자만)
        displayPattern: '031-720-1{ext}'  // 사람이 읽는 형식 (툴팁 표시용)
    },

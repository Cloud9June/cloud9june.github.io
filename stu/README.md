...
배포 경로는 `/stu/` 입니다. v1 파일은 모두 제거되었습니다.
(정리는 `정리-실행.bat` 이 했고, 이전 파일은 git 기록에 남아 있습니다.)
...
| 명부 입력 | 선생님이 전부 수기 입력 | 본인이 등록, 교사는 승인 후 활성 (v2.1) |
...
│  ├─ register.js        본인 등록 화면 (첫 로그인)
│  ├─ app.js             학생 앱 진입점
│  └─ admin.js           관리 콘솔
├─ tests/                보안 규칙 테스트 (배포에 꼭 필요하진 않음)
├─ tools/
│  └─ 명부_내보내기.gs   기존 시트 → 붙여넣기용 텍스트
├─ icons/                앱 아이콘 (v1 에서 그대로)
└─ video/                로그인 배경 영상 (v1 에서 그대로)
```

`README.md`, `firestore.rules`, `tests/`, `tools/` 는 개발용입니다.
서버에 함께 올라가도 문제는 없지만(민감한 값이 없습니다), 배포에서 빼도 앱은 정상 동작합니다.
...
### 3-2. 명부 만들기 — 본인 등록 방식 (v2.1)

**명부를 미리 넣을 필요가 없습니다.** 학생과 선생님이 첫 로그인 때 스스로 등록합니다.

- **학생**: 이름 · 학년 · 반 · 번호를 고르면 **즉시 사용 가능**
- **선생님**: 이름과 (선택) 담임 반을 신청하면 **승인 대기**.
  총관리자가 관리 콘솔에서 승인해야 교사 권한이 살아납니다.

권한(총관리자·관리자·담임·반장)은 **본인이 절대 스스로 줄 수 없습니다.**
보안 규칙이 `privilege == []` 를 강제하고, 권한 부여는 총관리자만 합니다.

등록은 **딱 한 번**만 됩니다. 규칙의 `create` 는 문서가 없을 때만 성공하므로
저장 후에는 본인이 학년·반을 바꿀 수 없습니다. 오타는 관리 콘솔에서 고쳐 주세요.

> **주의**: 학교 계정 이메일에 학번 정보가 없어, 학생이 신고한 학년·반·번호를
> 검증할 방법이 서버에는 없습니다. 관리 콘솔 명부 화면 위쪽의
> **"확인이 필요한 항목"** 에 번호 중복·누락이 자동으로 뜨니 학기 초에 한 번
> 훑어봐 주세요. 담임 선생님께 반 명단을 확인받는 것이 가장 확실합니다.

기존 시트 명부가 있다면 여전히 일괄 등록도 됩니다.
`tools/명부_내보내기.gs` 실행 → 관리 콘솔 → 명부·권한 → **일괄 등록**.

> **딱 하나, 첫 총관리자는 손으로 만들어야 합니다.** 아무도 총관리자가 아니면
> 선생님 승인을 할 사람이 없기 때문입니다. Firebase 콘솔 → Firestore 에서
> 본인 문서를 한 개 만들어 주세요. (학생·선생님은 만들 필요 없습니다)
>
...
**붙여넣기 전에 테스트를 먼저 돌리세요.** `tests/` 에 63개 시나리오가 준비되어 있습니다.
...
| 7. 본인 등록 | 권한 자가 부여 차단, 1회 제한, 남의 문서 생성 차단, 값 범위 |
| 8. 승인 대기 | pending 교사의 반별 피드·명부 접근 차단, 총관리자만 승인 |

### 3-4. 파일 올리기

커밋·푸시(또는 업로드) 후 `https://eduinfo.sungil-i.kr/stu/` 로 확인합니다.

**v1 의 서비스워커가 브라우저에 남아 있습니다.** 배포 직후 한 번은
`Ctrl+Shift+R` 로 강력 새로고침해야 새 화면이 나옵니다.

옛 화면이 계속 보이는지 확인하는 법: **더보기 화면 맨 아래**를 보세요.

- `S:NOW v2.1.0 · 성일정보고등학교` → 정상
- `⚠ 화면 파일이 낡았습니다 (앱 v2.1.0 · 화면 v2.0.2)` → 캐시가 낡음.
  바로 아래 **[캐시 비우고 새로고침]** 버튼을 누르면 해결됩니다.

주소에 `?sw=off` 를 붙여 열어도 서비스워커와 캐시를 모두 비울 수 있습니다.
